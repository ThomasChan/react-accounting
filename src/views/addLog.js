...
const AddLog = Form.create()(class AddLogClass extends Component {
  ...
  handleSubmit(e) {
    ...
    let {type, amount, description, date_year, date} = this.state
    sendData(`/api/AddLog`, {type, amount, description, date_year, date}, (res) => {
  ...
  render() {
    ...
    return <Col span={4}><Form horizontal onSubmit={e => this.handleSubmit(e)}>
      <FormItem>
      ...
      <FormItem>
        <MonthPicker format={'YYYY/MM'} onChange={(date, dateString) => this.onMonthChange(date, dateString)} />
      </FormItem>
      <FormItem>
        <Button type="primary" htmlType="submit" size="large" loading={this.state.submiting}>Add New Log</Button>
      </FormItem>
    </Form></Col>
  }
})
...
export default { AddLog, AddLogReducer }
